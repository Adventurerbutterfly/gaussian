...
def normalize_route(route_lines):
    return " ".join(" ".join(line.strip() for line in route_lines).split()).strip()
...
def replace_canonical_files(canonical_log: Path, trial):
    canonical_com = canonical_log.with_suffix(".com")
    canonical_chk = canonical_log.with_suffix(".chk")

    trial_opt_com = Path(trial["opt_inp"])
    trial_freq_log = Path(trial["freq_log"])
    trial_chk = Path(trial["chk"])

    backup_if_exists(canonical_log)
    backup_if_exists(canonical_com)
    backup_if_exists(canonical_chk)

    shutil.copy2(trial_freq_log, canonical_log)
    shutil.copy2(trial_opt_com, canonical_com)
    if trial_chk.exists():
        shutil.copy2(trial_chk, canonical_chk)
...
def main():
    ...
    rows = ["file\tchosen_tag\tamp\tstatus\tnimag\tminfreq\treplaced"]
    success = 0

    for log in logs:
        print(f"\n=== {log.name} ===")
        if not log.exists():
            rows.append(f"{log}\t-\t-\tmissing_log\t-\t-\tno")
            continue
        ...
        replaced = "no"
        if best["status"] == "ok":
            replace_canonical_files(log, best)
            replaced = "yes"
            success += 1

        rows.append(
            f'{log}\t{best.get("tag","-")}\t{best.get("amp","-")}\t{best.get("status","-")}\t'
            f'{best.get("nimag","-")}\t{best.get("minfreq","-")}\t{replaced}'
        )

        print(
            f"chosen={best.get('tag')}  amp={best.get('amp')}  "
            f"status={best.get('status')}  nimag={best.get('nimag')}  "
            f"minfreq={best.get('minfreq')}  replaced={replaced}"
        )
